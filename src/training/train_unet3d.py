...
class CombinedLoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        # Add channel dimension to target for BCE
        if target.dim() == 4:
...
def train_one_epoch(model: nn.Module,
    ...
    for batch_idx, (features, target, metadata) in enumerate(pbar):
        ...
        # Reshape features: [B, T, C, H, W] -> [B, C, T, H, W]
        ...
        # Forward pass
        output = model(features)  # [B, 1, T, H, W]
...
def main():
    ...
    print("="*80)
    print("WILDFIRE SPREAD PREDICTION - 3D U-NET TRAINING")
    print("="*80)
    print(f"\nConfiguration:")
